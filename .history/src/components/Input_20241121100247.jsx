...
export default function Input({ label, type, emptydata }) {
  ...
  if (type === "message") {
    return (
      <textarea
        className={twMerge(
          "w-full border-b-[1px] border-[#1B1D23] pb-5 pl-[2.125rem] placeholder:border placeholder:text-[1.25rem] placeholder:text-[#c8ccd8] focus:border-b-4 focus:border-b-[#1b1d23] focus:placeholder:text-[#1b1d23] focus-visible:outline-none",
          errorCLass,
        )}
        name="message"
        placeholder={label}
